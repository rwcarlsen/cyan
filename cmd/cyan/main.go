...
func doInv(cmd string, args []string) {
	...
	nucs := fs.String("nucs", "", "filter by a comma separated `nuclide`s")
	fs.Parse(args)
	if fs.NArg() < 1 {
		log.Fatal("must specify a prototype")
	}
	...
	proto := fs.Arg(0)
	s := `SELECT tl.Time AS Time,IFNULL(sub.qty,0) AS Quantity FROM timelist as tl
          LEFT JOIN (
			 SELECT tl.Time as time,TOTAL(inv.Quantity) AS qty FROM timelist as tl
...
func nuclidefilter(nucs string) string {
	...
	filter := fmt.Sprintf(" c.nucid IN (%v", int(nnucs[0]))
